...
| 4 | **Policy Model (生成)** | 21 秒 | **`41.66` 秒** | **`1.98x`** | 640x480 x 17f 動画 + 16x10 アクション出力。サンプリング + VAE デコード |
| - | ┗ デノイズサンプリング | 内訳非公開 | `33.84` 秒 | — | 30 steps |
| - | ┗ VAE デコード | 内訳非公開 | `7.49` 秒 | — | 17f ビデオ復元 (upsample_3 torch.compile 最適化) |
| - | ┗ (参考) 入力観測エンコード | 内訳非公開 | `81.57` 秒 | — | 上記「生成」には含まない。下記 NOTE 参照 |
...
> * **Policy Model は目標（対記事 1.5 倍以内 = 31.5 秒）に未達です**（1.98 倍）。T2I / T2V / I2V は目標内です。
>   初期状態（約 1965 秒）比では 47 倍の改善にあたります。

> [!IMPORTANT]
> **Policy Model の「生成 41.66 秒」に入力観測動画のエンコード（conditioning、本環境実測 81.57 秒）は含まれません。**
> 記事側は「モデル常駐後 21 秒」とのみ記載され内訳が非公開のため、記事側が同エンコードを含むかは不明です。
> ただし本環境の conditioning 単体（81.57 秒）が記事の総時間 21 秒を大きく超えること、および
> T2I / T2V / I2V（重い入力エンコードを持たない）での実測比が 1.2〜1.5 倍にとどまることから、
> **記事の 21 秒も入力エンコードを含まない「生成のみ」の値と推定**し、上表はその前提で比較しています。
> 参考として、入力エンコードを含めた同期総和は **124.91 秒**です。
>
...
> ```
> 入力エンコードを含む 124.91 秒の測定は `--policy-sync-profile`（condition-cache なし）で再現できます。
