...
## DLS-024
- **date**: 2026-07-28
- **what**: 5分発表用の元マークダウン（TLDR / cosmos3紹介 / 元記事 / 初回ベンチ / 最終ベンチ / 高速化寄与Top3 の6節構成）を docs/ に置き、掲載数値はすべてリポジトリ内実測記録（README §2-4、final report、result/）または出典URL付き外部情報（価格）に紐づける。価格は「変動あり」の注記付きで提示する
- **why**:
  - origin: user_request
  - business: プロジェクト成果を5分で対外発表するための一次資料が必要。DLS-006 の教訓（一次出典のない数値の対外主張が誤報を生む）に従い、TLDR で提示する両機の価格を出典付きで固定する。DLS-016 時点で「価格の一次出典は未固定」だった欠落を発表資料の範囲で解消する
  - constraint: 価格は市況で変動する（DGX Spark は memory supply 制約で 2026-02 に $3,999→$4,699 へ改定、EVO-X2 128GB は発売時 $1,999 に対し現在実売 $2,229〜）。倍率・秒数は既存の検証済み公表値（DLS-007/016/018/021）のみを転記し、新規測定は行わない
- **where**: docs/presentation_5min_cosmos3_rocm.md（新規）、README.md §2-4（数値の転記元）、docs/cosmos3_rocm_policy_optimization_final_report.md（初回ベンチ 1965 秒と最適化内訳の転記元）
- **sources**: なし（本セッションの /dls-plan 判断。価格出典は資料本文にURLで記載）
- **requested_by**: ユーザー（「このプロジェクトの内容を5分程度で発表するための元になるマークダウンを作成」6節構成指定、2026-07-28）
- **depends_on**: DLS-006, DLS-007, DLS-011, DLS-016, DLS-018, DLS-021, DLS-023
- **affects**: なし（既存の公表値・注記を転記するのみで、判断の変更はない）
- **rejected_alternatives**:
  - 価格をプレースホルダにしてユーザーが埋める: TLDR の核である価格提示が未完成のまま渡る。Web検索で出典が取得できたため不要
  - 何もしない（README §2-4 をそのまま発表に使う）: 指定の6節構成・TLDR・初回ベンチ対比が README に無く、5分発表の流れにならないため不採用
  - スライドツール形式（Marp 等のページ区切り）で作る: ユーザー指定は「元になるマークダウン」であり、ツール前提を持ち込むと汎用性が下がる。dormant（スライド化の要望が出たら再評価）
- **commits**:
  - baseline: 0aa46f7
- **assumption**: 価格は 2026-07-28 時点の検索結果で、DRAM 市況により変動する（confidence: medium。資料に取得日と出典URLを明記して緩和）。高速化寄与 Top3 の選定（同期排除+HIP Graphs / VAE 部分コンパイル / 厳密 und branch cache）は、モード別に文書化された実測効果の大きさに基づく編集判断であり、寄与の厳密な分離測定は存在しない（confidence: high。各項目の個別実測値は final report と README に記録済み）
