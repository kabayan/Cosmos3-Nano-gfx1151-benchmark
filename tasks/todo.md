...
- [ ] CFG 条件チューニングの仮説選定（`/dls-plan`）: 公式 guidance 条件で「価格差 2.0 以内」（DLS-016 で確定した合否軸）に入れるための候補を採否判定する。候補: (1) und branch cache の 2 スロット化（cond/uncond 各 1、厳密キャッシュのまま read 化回復。概算で T2I 5.25x→≈2.4x / I2V 11.33x→≈2.6x、**それでも 2.0 超**）、(2) T2V への und cache 適用（現行未使用）、(3) per-call 約 20% の追加短縮策（2.0 到達の本丸。CFG 条件のハードウェア素比 ≈2.5 倍が壁）、(4) 何もしない（guidance 1.0 値を条件明記で主提示に残す）。前提知見は DLS-016 / raw `20260728_doc_guidance_official_measurement.md` §8
