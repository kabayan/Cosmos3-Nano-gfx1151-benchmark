...
- [ ] **und branch cache の 2 スロット化を実装する（DLS-017 採用案）**: CFG 下で cond/uncond の署名が交互に不一致となり全 write 化している（140 calls / 140 writes / 0 reads）状態を、スロットを 2 つ持たせて read 化に戻す。厳密キャッシュのまま（近似ではない）。実装先は `third_party/diffusers/src/diffusers/models/transformers/transformer_cosmos3.py`（`_und_branch_cache` L496-540 の単一スロット構造、L871-900 の署名判定）で、**イメージ同梱 `/opt/diffusers` と同期させる**（乖離させない）。検証: `result/guidance_official_20260728/run_commands.sh` と同一条件で T2I/I2V を再測定し、(a) cache stats が read 主体に変わること、(b) 出力が 2 スロット化前と一致すること（厳密性の確認）、(c) 倍率の回復幅（概算 T2I 5.25x→≈2.4x / I2V 11.33x→≈2.6x、DLS-017 assumption の反証機会）。**2.0 到達は目的ではない**（DLS-017 で CFG 条件の 2.0 は同一計算内容の制約下では非現実的と確定）
- [ ] README §2 の対外表現の更新: 合否軸を「対記事 1.5 倍以内」から「価格差 2.0 以内」（DLS-016）に差し替え、倍率表を guidance 1.0 / 公式デフォルト両条件併記にする（1.0: 1.23x/1.46x/1.47x、公式値: 5.25x/2.53x/11.33x）。Policy 1.98x は新基準の内側（conditioning 込み 2.02x は境界上）。精度は素の提示に凍結（ユーザー決定 2026-07-28）: golden MSE 実測 0.126〜0.134 と「記事側も step 6-7 で per-step 0.05 超過」（記事チャート、raw doc §5-6）を併記。「対論文比」列見出しと `docs/cosmos3_rocm_optimization_analysis.md` L17-19 の「論文値」表記は基準がすべて記事値のため独立に修正可能（DLS-006 確定事項）。CUDA 参照 run 待ちの帰属注記は当分不可の前提（2026-07-28 ユーザー決定）に変更
