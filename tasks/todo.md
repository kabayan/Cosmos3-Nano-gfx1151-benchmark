...
- [ ] README §2 の対外表現の更新: 合否軸を「対記事 1.5 倍以内」から「価格差 2.0 以内」（DLS-016）に差し替え、倍率表を guidance 1.0 / 公式デフォルト両条件併記にする（1.0: 1.23x/1.46x/1.47x、公式値は DLS-018 の 2 スロット化後: **T2I 2.25x / T2V 2.53x / I2V 2.69x**、根拠 `result/guidance_2slot_20260728/`）。Policy 1.98x は新基準の内側（conditioning 込み 2.02x は境界上）。精度は素の提示に凍結（ユーザー決定 2026-07-28）: golden MSE 実測 0.126〜0.134 と「記事側も step 6-7 で per-step 0.05 超過」（記事チャート、raw doc §5-6）を併記。「対論文比」列見出しと `docs/cosmos3_rocm_optimization_analysis.md` L17-19 の「論文値」表記は基準がすべて記事値のため独立に修正可能（DLS-006 確定事項）。§4 ピーク TFLOPS に形状依存の注記（DLS-017 副次発見: 実運用 GEMM 36.11 TF > bench_peak 20.91 TF）。CUDA 参照 run 待ちの帰属注記は当分不可の前提（2026-07-28 ユーザー決定）に変更
