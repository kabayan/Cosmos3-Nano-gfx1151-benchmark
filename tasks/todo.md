...
- [x] ~~E4: v1 重み ROCm golden 照合~~ → **完了・仮説棄却**（DLS-014）。v1 は 0.248372 で v2（0.126〜0.134）の約 2 倍悪化。副産物として DLS-013 の「別 checkpoint」判定を訂正（全 1165 tensor 比較で相対差の中央値 0.0128・43 tensor は bitwise 一致 = 同一系統の継続学習、差は生成経路に集中）。原本: `.claude/.dls/raw/20260728_doc_e4_v1_checkpoint_golden_verification.md`
...
- [x] ~~MIOpen カーネルキャッシュのホストマウント~~ → **実装済み**（DLS-015）。MIOpen / inductor / triton の 3 種を `scripts/run_cosmos_framework_policy_docker.sh` でホストへ持ち越し（`CACHE_DIRS=0` で無効化可）。効果測定は上記 Active 項目に残す
